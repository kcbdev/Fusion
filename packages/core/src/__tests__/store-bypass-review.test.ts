...
describe("TaskStore.bypassFailedPreMergeReviewStep", () => {
  let rootDir: string;
  let globalDir: string;
  let store: TaskStore;

  beforeEach(async () => {
    rootDir = await mkdtemp(join(tmpdir(), "store-bypass-review-"));
  ...
  it("records a run-audit event for the bypass", async () => {
    ...
    const events = store.getRunAuditEvents({ taskId: "FN-BYP-002" });
